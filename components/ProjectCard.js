...
export const ProjectCard = (props) => {
    ...
    return ( 
        <div className="showcase-container mt-5 d-flex w-100">
         <div className="content px-4 d-flex flex-column justify-content-start align-items-center ">
         <h2 className="mb-3">{project.title}</h2>
               { !showDetails ? <p className="w-50 text-center">{project.blurb}</p> :<p className="w-75 text-left">{project.details}</p>}
               
               <div className="image-container">
                  <img  style={{marginBottom: "-1em"}} className="w-100" src={project.image} alt={project.title} />
               </div>
